...
// TODO: do this later. The pem key should be base 64 encoded
...
/// retrieve a personal access token from the .env file
/// returns a personal access token
pub fn personal_access_token() -> String {
    dotenv().ok();
    return env::var("GH_PAT").expect("GH_PAT to be set");
}
